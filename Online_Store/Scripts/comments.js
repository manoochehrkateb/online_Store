...
function Post(el) {
    ...
    function createCommentElements(comments) {
        ...
        if (!comments.length) {
            return $('<div class="no-comments">بدون نظر</div>');
        }

        return comments.reduce(function (commentEls, comment) {
            //var timestamp = Date.parse(comment.PostedDate);
            //var d; 
            ...
            var el =
                $('<div class="comment">')
                    .data('comment-id', comment.ProductIdForComment)
                    .append($('<p class="details">').append(comment.Author || 'مهمان').append(' در ' + new Date(comment.PostedDate).toLocaleString()))
    ...
    function PostCommentService() {
        ...
        function call(postId, method, data) {
            return $.ajax({
                ...
                url: ['/api/products', postId, 'comments'].join('/'),
                type: method,
